"""
Test script for enhanced market making HFT strategy vs original
Compares performance and validates improvements
"""
import sys
import os
import logging
...
def test_enhanced_vs_original_market_making():
    """Test enhanced market making strategy vs original"""
    ...
    test_configs = [
        ...
        {
            'name': 'Enhanced Market Making',
            'strategy_class': 'EnhancedMarketMakingHFTStrategy',
            'params': {
                'spread_width': 0.0002,
                'max_inventory': 5,
                'inventory_rebalance_threshold': 2,
    ...
    for config in test_configs:
        ...
        # Load data
        logger.info(f"Loading data for CL (futures, 1h)...")
        loaded_data = backtest_engine.load_data('CL', 'futures', '1h')
        
        if loaded_data is None or loaded_data.empty:
            logger.error(f"Failed to load data for CL")
            results.append({
                'name': config['name'],
        ...
        if result and isinstance(result, dict):
            logger.info(f"Backtest completed for {config['name']}")
            ...
            logger.error(f"Backtest failed for {config['name']}")
            results.append({
                'name': config['name'],
    ...
    if len(results) == 2:
        ...
        logger.info(f"Original Strategy:")
        ...
        if enhanced['total_return'] > original['total_return'] and enhanced['max_drawdown'] <= original['max_drawdown']:
            ...
            logger.info(f"\n📈 ENHANCED STRATEGY has better returns")
        elif enhanced['max_drawdown'] < original['max_drawdown']:
            logger.info(f"\n🛡️ ENHANCED STRATEGY has lower risk")
        else:
...
if __name__ == "__main__":
    print("Starting Enhanced vs Original Market Making Strategy Test")
    print("=" * 80)
    
    results = test_enhanced_vs_original_market_making()
    ...
    if results:
        print(f"Tested {len(results)} strategies successfully")
