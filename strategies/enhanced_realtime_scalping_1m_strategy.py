...
class EnhancedRealtimeScalping1MStrategy(bt.Strategy):
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            ...
            if portfolio_change > 0:
                self.logger.info(f"*** 1M SCALPING FINAL RESULT: PROFIT ${portfolio_change:.2f} (+{portfolio_change_pct:.2f}%) ***")
            elif portfolio_change < 0:
                self.logger.info(f"*** 1M SCALPING FINAL RESULT: LOSS ${portfolio_change:.2f} ({portfolio_change_pct:.2f}%) ***")
            else:
                self.logger.info(f"*** 1M SCALPING FINAL RESULT: BREAK EVEN ${portfolio_change:.2f} (0.00%) ***")
