...
class EnhancedRealtimeScalping1MStrategy(bt.Strategy):
        ...
        if not self.position:
            ...
            if (signals['buy_score'] > self.p.signal_strength_threshold and 
                ...
                try:
                    ...
                    if portfolio_change > 0:
                        self.logger.info(f"*** 1M SCALPING PROFIT: ${portfolio_change:.2f} (+{portfolio_change_pct:.2f}%) ***")
                    elif portfolio_change < 0:
                        self.logger.info(f"*** 1M SCALPING LOSS: ${portfolio_change:.2f} ({portfolio_change_pct:.2f}%) ***")
                    else:
                        self.logger.info(f"*** 1M SCALPING BREAK EVEN: ${portfolio_change:.2f} (0.00%) ***")
            ...
            # Check for sell signals
                ...
                try:
                    ...
                    self.logger.info(f"*** 1M SCALPING PORTFOLIO VALUE AFTER SELL ORDER ***")
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            ...
            if portfolio_change > 0:
                self.logger.info(f"*** 1M SCALPING TRADE RESULT: PROFIT ${portfolio_change:.2f} (+{portfolio_change_pct:.2f}%) ***")
            elif portfolio_change < 0:
                self.logger.info(f"*** 1M SCALPING TRADE RESULT: LOSS ${portfolio_change:.2f} ({portfolio_change_pct:.2f}%) ***")
            else:
                self.logger.info(f"*** 1M SCALPING TRADE RESULT: BREAK EVEN ${portfolio_change:.2f} (0.00%) ***")
